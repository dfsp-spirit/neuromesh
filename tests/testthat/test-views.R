...
test_that("all anatomical view cameras produce usable output", {
    ...
    opts <- scimesh::render_options(width = 16, height = 16)
    ...
    for (v in views) {
        cam <- v(verts)
        img <- scimesh::render_mesh(verts, tris, camera = cam, options = opts)
...
test_that("render_hemisphere_views with colorbar", {
    ...
    cbar <- scimesh::colorbar_horizontal(scimesh::viridis_colormap, width = 200, height = 30)
