...
.view_to_camera <- function(view_name, mesh) {
    ...
    cam <- scimesh::camera_auto(fit_mesh, direction = cam_spec$dir,
...
.apply_colormap <- function(morph_data, makecmap_options) {
    ...
    if (is.null(col_fn)) {
        col_fn <- scimesh::viridis_colormap
    }
...
vis.subject.morph.native <- function(subjects_dir, subject_id, measure,
    ...
    if (is.null(cmap$colFn))  cmap$colFn  <- scimesh::viridis_colormap
    ...
    # ---- Render options ----
    ropts <- scimesh::render_options(width = width, height = height,
    ...
    for (vn in view_names) {
        ...
        img <- scimesh::render_scene(scene, cam, ropts)
    ...
    if (isTRUE(draw_colorbar) || is.character(draw_colorbar)) {
        ...
        if (use_horizontal) {
            ...
            cbar <- scimesh::colorbar_vertical(
                cmap$colFn,
                n_colors = cmap$n,
    ...
    # ---- Compose layout ----
    scimesh::compose_layout(images, colorbar = cbar,
