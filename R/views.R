...
view_lateral_left <- function(mesh, fov = 45, margin = 1.1) {
    scimesh::camera_auto(mesh, direction = c(-1, 0, 0), up = c(0, 0, 1),
...
view_dorsal <- function(mesh, fov = 45, margin = 1.1) {
    scimesh::camera_auto(mesh, direction = c(0, 0, 1), up = c(0, 1, 0),
                fov = fov, margin = margin)
}
...
view_anterior <- function(mesh, fov = 45, margin = 1.1) {
    scimesh::camera_auto(mesh, direction = c(0, 1, 0), up = c(0, 0, 1),
                fov = fov, margin = margin)
}
...
render_hemisphere_views <- function(mesh, hemisphere = c("left", "right"),
    ...
    if (is.null(options)) {
        options <- scimesh::render_options(width = width, height = height,
